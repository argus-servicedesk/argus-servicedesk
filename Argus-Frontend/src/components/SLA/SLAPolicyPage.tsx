...
export default function SLAPolicyPage() {
  ...
  const overallCompliance = rows.length > 0
    ? rows.reduce((acc, row) => acc + row.pct, 0) / rows.length
    : 100;
        ...
        <SNCollapsibleSection title="SLA definitions">
          <div className="overflow-x-auto bg-white">
            <table className="w-full border-collapse text-left">
              <thead>
                <tr style={{ background: '#f7f8fa', borderBottom: `1px solid ${sn.border}` }}>
                  {['Priority', 'Description', 'Response Target', 'Resolution Target', 'Breaches', 'Compliance %', 'Actions'].map((heading) => (
                    <th key={heading} className="px-5 py-3 text-[13px] font-bold uppercase tracking-wide" style={{ color: sn.label }}>
                      {heading}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {isLoading ? (
                  ...
                  <tr key={row.priority} style={{ borderBottom: `1px solid ${sn.border}` }}>
                    ...
                    <td className="px-5 py-4 text-[14px]" style={{ color: sn.text }}>{row.meta.description}</td>
                    <td className="px-5 py-4 font-mono text-[14px]" style={{ color: sn.text }}>{fmtMins(row.responseTarget)}</td>
                    <td className="px-5 py-4 font-mono text-[14px]" style={{ color: sn.text }}>{fmtMins(row.resolutionTarget)}</td>
                    <td className="px-5 py-4 font-mono text-[14px]" style={{ color: row.breaches > 0 ? sn.critical : '#067647' }}>
